...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($_POST['phone']) || empty($_POST['planid']) || empty($_POST['networkid'])) {
        ...
        function buySmilePlan($token, $payload) {
            // ✅ Smile uses different endpoint
            $url = "https://atpay.ng/api/smile/";
            $ch = curl_init($url);
        ...
        try {
            ...
            if (empty($token)) {
                ...
                // ✅ Smile payload (no ported_number needed)
                $payload = [
                    "network"   => $network,
                    "smile_number" => $phone, // Smile-specific field
                    "plan"      => $planid
                ];
                
                $result = buySmilePlan($token, $payload);
                $response = $result;
            }
...
<head>
  ...
  <title>atPay - Smile Transaction Receipt</title>
 ...
 </style> <!-- ✅ Reuse your receipt styling -->
</head>
<body>
<?php include "../../../../include/user_top_navbar.php" ?>
  <main class="main-container">
    <div class="receipt">
      <div class="receipt-header">
        ...
        <div class="receipt-title">Smile Transaction Receipt</div>
      </div>
      ...
      <div class="receipt-body">
        <div class="status-section">
          <?php if ($response && ($response['status'] ?? 'failed') === 'success'): ?>
            ...
            <div class="status-description">Your Smile plan purchase was completed successfully.</div>
          <?php else: ?>
        ...
        <div class="details-section">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value">Smile Data Purchase</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">Network:</span>
            <span class="detail-value">Smile</span>
          </div>
          <div class="detail-row">
          ...
          <div class="detail-row">
            <span class="detail-label">Smile Number:</span>
            <span class="detail-value"><?= htmlspecialchars($response['smile_number'] ?? '') ?></span>
          </div>
          <div class="detail-row">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      <?php if ($response): ?>
        Swal.fire({
          title: "<?= ($response['status'] ?? 'failed') === 'success' ? 'Smile Purchase Successful!' : 'Smile Purchase Failed!' ?>",
          html: `
            <p><strong>Plan:</strong> <?= htmlspecialchars($response['plan_name'] ?? '') ?></p>
            <p><strong>Smile Number:</strong> <?= htmlspecialchars($response['smile_number'] ?? '') ?></p>
